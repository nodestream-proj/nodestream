...
def test_assume_role_and_get_credentials(mocker):
    subject = AwsClientFactory(assume_role_arn="arn:aws:iam::123456789012:role/test")
    ...
    credentials = subject.assume_role_and_get_credentials()
    assert credentials["access_key"] == "test_access_key"


def test_get_boto_session_with_refreshable_credentials(mocker):
    # mock assume_role_and_get_credentials and assert that a session was created with refreshable credentials
    subject = AwsClientFactory(assume_role_arn="arn:aws:iam::123456789012:role/test")
    subject.assume_role_and_get_credentials = mocker.MagicMock(
        return_value={
    ...
    session = subject.get_boto_session_with_refreshable_credentials()
    assert session._credentials.method == "sts-assume-role"


def test_assume_role_if_supplied_and_get_session(mocker):
    ...
    subject = AwsClientFactory(assume_role_arn="arn:aws:iam::123456789012:role/test")
    subject.get_boto_session_with_refreshable_credentials = mocker.MagicMock(
        return_value="test_session"
    )
    session = subject.assume_role_if_supplied_and_get_session()
    assert session == "test_session"


def test_assume_role_if_supplied_and_get_session_no_role_arn(mocker):
    # create a AwsClientFactory without a role arn and mock get_boto_session_with_refreshable_credentials.
    # assert that a the result of get_boto_session_with_refreshable_credentials is what is returned.
    subject = AwsClientFactory()
    subject.get_boto_session_with_refreshable_credentials = mocker.MagicMock()
    session = subject.assume_role_if_supplied_and_get_session()
    subject.get_boto_session_with_refreshable_credentials.assert_not_called()
    assert isinstance(session, Session)


def test_make_client(mocker):
    ...
    subject = AwsClientFactory()
    session = Session()
    subject.assume_role_if_supplied_and_get_session = mocker.MagicMock(
        return_value=Session()
    )
    result = subject.make_client("s3")
