...
def test_convert_array():
    assert convert_array([TestObject, TestObject, TestObject], leave_untouched) == [TestObject, TestObject, TestObject]

def test_convert_map():
    assert convert_map({"key1": TestObject, "key2": TestObject}, leave_untouched) == {"key1": TestObject, "key2": TestObject}
...
DESCRIPTOR_HANDLER_TESTS = [
    ...
    ({"L": [{"S": "TestString"}, {"BS": ["abcdef", "xyz123"]}]}, [ "TestString", [b'abcdef', b'xyz123'] ]),
]
...
def test_dynamo_data_class_initialization_from_raw_data():
    ...
    assert dynamo_data_b.python_format == {
        "key1": "TestString", 
        "key2": [100, 1.0]
    }

TEST_DYNAMO_RECORD = {
    "number": TEST_DYNAMO_OBJECT_A,
    "metadata": TEST_DYNAMO_OBJECT_B
}
...
def test_dynamo_extractor_initialization_without_args():
    with patch("nodestream.pipeline.extractors.credential_utils.AwsClientFactory.make_client") as client_mock:
...
def test_dynamo_extractor_table_scanner():
    with patch("nodestream.pipeline.extractors.credential_utils.AwsClientFactory.make_client") as client_mock:
        ...
        extractor.client = MockResponder(TEST_DATA)
