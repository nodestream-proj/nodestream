...
TEST_SOURCE_NODE_INTERPRETATION = SourceNodeInterpretation(node_type="Test", key={"test_key": "test_value"})
TEST_RELATIONSHIP_INTERPRETATION = RelationshipInterpretation(node_type="Test", node_key={"test_key": "test_value"}, relationship_type="TEST_REL")
TEST_PROPERTIES_INTERPRETATION = PropertiesInterpretation(properties={"test_property": "thing"})
...
def test_multisequence_interpretation_pass_assigns_source_nodes():
    subject_a = MultiSequenceInterpretationPass(*[SingleSequenceInterpretationPass(*SOURCE_NODE_INTERPRETATION_PASS) for _ in range(2)])
    assert subject_a.assigns_source_nodes
    assert subject_a.should_be_distinct
    
    subject_b = MultiSequenceInterpretationPass(*[SingleSequenceInterpretationPass(*CONTEXT_INTERPRETATION_PASS) for _ in range(2)])
    assert subject_b.assigns_source_nodes is False
...
def test_interpretation_pass_exclusively_assigns_source_nodes():
    ...
    subject_c = SingleSequenceInterpretationPass(*INTERPRETATION_PASS_WITH_SOURCE_NODE_SWITCH)
...
def test_interpretation_pass_interpretation_ordering():
    subject_a = SingleSequenceInterpretationPass(*SOURCE_NODE_INTERPRETATION_PASS)
    assert is_ordered(subject_a.schema_ordered_interpretations, SOURCE_NODE_INTERPRETATION_PASS_ORDERED)
    subject_b = SingleSequenceInterpretationPass(*CONTEXT_INTERPRETATION_PASS)
    assert is_ordered(subject_b.schema_ordered_interpretations, CONTEXT_INTERPRETATION_PASS)
    subject_c = SingleSequenceInterpretationPass(*INTERPRETATION_PASS_WITH_SOURCE_NODE_SWITCH)
    assert is_ordered(subject_c.schema_ordered_interpretations, INTERPRETATION_PASS_WITH_SOURCE_NODE_SWITCH_ORDERED)
...
def test_multisequence_interpretation_pass_verifies_source_node_completeness():
    with pytest.raises(InterpretationPassError) as error:
        _ = InterpretationPass.from_file_data(SINGLE_SEQUENCE_MULTIPLE_SOURCE_NODE_GENERATORS)
        assert error.message == COMPLETENESS_ERROR_MESSAGE
    

    with pytest.raises(InterpretationPassError) as error:
        _ = InterpretationPass.from_file_data(MULTI_SEQUENCE_INCOMPLETE_SOURCE_NODE_GENERATORS)
