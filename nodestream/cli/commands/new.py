...
class New(NodestreamCommand):
    ...
    arguments = [
        argument("path", "the location of the project to create"),
        argument("name", "the name of the project to create", optional=True),
    ]
    options = [DATABASE_NAME_OPTION]

    async def handle_async(self):
        project_path = Path(self.argument("project"))
        project_name = self.option("name") or project_path.stem
