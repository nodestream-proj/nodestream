...
def test_class_loader_constructor(subject):
    result = subject.load_class(
        implementation="nodestream.tests.unit.pipeline.test_class_loader:SimpleClass",
...
def test_class_loader_declartive_init(subject):
    result = subject.load_class(
        implementation="nodestream.tests.unit.pipeline.test_class_loader:SimpleClassWithFactories",
        arguments={"argument": "test"},
    )
...
def test_class_loader_invalid_path_format(subject):
    with pytest.raises(InvalidClassPathError):
        subject.load_class(
            implementation="nodestream.tests.unit.pipeline.test_class_loader"
        )
...
def test_class_loader_invalid_path_missing_class(subject):
    with pytest.raises(InvalidClassPathError):
        subject.load_class(
            implementation="nodestream.tests.unit.pipeline.test_class_loader:ClassDoesNotExist"
        )
